...
CREATE POLICY "Admins can manage creators" ON public.creators FOR ALL USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Subscribers can view premium content" ON public.content 
  FOR SELECT USING (
    ...
    (is_premium = false OR 
     public.is_subscribed_to_creator(auth.uid(), creator_id) OR
...
CREATE INDEX idx_creators_featured ON public.creators(is_featured);
CREATE INDEX idx_content_creator_id ON public.content(creator_id);
CREATE INDEX idx_content_status ON public.content(status);
CREATE INDEX idx_content_premium ON public.content(is_premium);
...
CREATE INDEX idx_content_views_viewer_id ON public.content_views(viewer_id);
CREATE INDEX idx_tips_creator_id ON public.tips(creator_id);
CREATE INDEX idx_tips_sender_id ON public.tips(sender_id);