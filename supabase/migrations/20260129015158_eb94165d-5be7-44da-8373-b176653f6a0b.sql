...
CREATE OR REPLACE VIEW public.admin_niche_analytics AS
SELECT 
  c.category AS niche,
...
CREATE OR REPLACE VIEW public.admin_creator_revenue AS
SELECT 
  c.id AS creator_id,
  c.user_id,
  c.stage_name,
...
CREATE OR REPLACE VIEW public.admin_subscription_retention AS
...
CREATE OR REPLACE VIEW public.admin_platform_arpu AS
...
COMMENT ON VIEW public.admin_niche_analytics IS 'Analytics par catégorie/niche pour le dashboard admin';
COMMENT ON VIEW public.admin_creator_revenue IS 'Revenus détaillés par créateur pour le dashboard admin';
COMMENT ON VIEW public.admin_subscription_retention IS 'Métriques de rétention des abonnés par cohorte mensuelle';
COMMENT ON VIEW public.admin_platform_arpu IS 'ARPU mensuel de la plateforme avec breakdown par type de revenu';